...
// For more information on STARS structures look inside of header files.
{
    ...
    {
        printf("%d\n", argc);
        printf("spatial1d n block_size maxrank oversample tol beta nu "
                "scheme\n");
        exit(1);
    }
    ...
    char *scheme = argv[8];
    int onfly = 0;
    printf("\nn=%d bs=%d mr=%d os=%d tol=%e beta=%f nu=%f scheme=%s\n",
            N, block_size, maxrank, oversample, tol, beta, nu, scheme);
    // Setting random seed
    srand(100);
    ...
    char symm = 'S', dtype = 'd';
    starsh_application((void **)&data, &kernel, N, dtype, "spatial1d", "exp",
